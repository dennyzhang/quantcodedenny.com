...
5. Optional: Suggest a **cool-down action** if the intent fails the guardrail (e.g., wait 1 day, review research, check valuation).  
...
```
I want to buy RKLB with $45 in the incoming weeks
```
...
Maintain a basket of sector leaders (clear moat, dominant position). Use LLMs to monitor valuation and sentiment swings, alerting on attractive pullback entries.

Example: UNH dip opportunity on early aug
